...
```
Saving M type matrices to perform rarefaction 
...
Performing rarefaction based on the region with less citations. This is used to control for the different number of workds citing each single author papers from different regions. We are gonna use 3000 references as the minimum sample to use in the bootstrap procedure (without replacement)
...
dim(M_citations_EastAsia)
dim(M_citations_EU)
dim(M_citations_la)
dim(M_citations_MiddleAfrica)
dim(M_citations_Sahara)
dim(M_citations_usa_can)
