...
We used the following terms for searching papers in WoS

`WC = Ecology AND CU = (Hungary OR Belarus OR Austria OR Serbia OR Switzerland OR Germany OR Andorra OR Bulgaria OR (United Kingdom) OR UK OR England OR Scotland OR Wales OR France OR Montenegro OR Luxembourg OR Italy OR Denmark OR Finland OR Slovakia OR Norway OR Ireland OR Spain OR Malta OR Ukraine OR Croatia OR Moldova OR Monaco OR Liechtenstein OR Poland OR Iceland OR (San Marino) OR (Bosnia and Herzegovina) OR Albania OR Lithuania OR (North Macedonia) OR Slovenia OR Romania OR Latvia OR Netherlands OR (Russian Federation) OR Estonia OR Belgium OR (Czech Republic) OR Czechia OR Greece OR Portugal OR Sweden) and NATURE ECOLOGY EVOLUTION or TRENDS IN ECOLOGY EVOLUTION or ECOLOGY or ECOLOGY LETTERS or METHODS IN ECOLOGY AND EVOLUTION or ECOLOGICAL APPLICATIONS or GLOBAL ECOLOGY AND BIOGEOGRAPHY or JOURNAL OF APPLIED ECOLOGY or JOURNAL OF ANIMAL ECOLOGY or BEHAVIORAL ECOLOGY or GLOBAL CHANGE BIOLOGY or FUNCTIONAL ECOLOGY or ECOSPHERE or ECOSYSTEMS or MOLECULAR ECOLOGY or ECOGRAPHY or APPLIED VEGETATION SCIENCE or BIODIVERSITY AND CONSERVATION or ECOLOGICAL MODELLING or MICROBIAL ECOLOGY or BIOLOGICAL CONSERVATION or JOURNAL OF BIOGEOGRAPHY or OECOLOGIA or RESTORATION ECOLOGY or JOURNAL OF VEGETATION SCIENCE or OIKOS or LANDSCAPE ECOLOGY or GLOBAL ECOLOGY AND CONSERVATION or FRESHWATER BIOLOGY or ANIMAL CONSERVATION or CONSERVATION BIOLOGY or JOURNAL OF ECOLOGY or DIVERSITY AND DISTRIBUTIONS`
...
```{r}
eu_names <- toupper(c("Hungary", "Belarus", "Austria", "Serbia", "Switzerland", "Germany", "Andorra", "Bulgaria", "United Kingdom", "UK", "England", "Scotland", "Wales", "France", "Montenegro", "Luxembourg",  "Italy", "Denmark", "Finland", "Slovakia", "Norway", "Ireland", "Spain", "Malta", "Ukraine", "Croatia", "Moldova", "Monaco", "Liechtenstein", "Poland", "Iceland", "San Marino", "Bosnia and Herzegovina", "Albania", "Lithuania", "North Macedonia", "Slovenia", "Romania", "Latvia", "Netherlands", "Russian Federation", "Estonia", "Belgium", "Czech Republic", "Czechia", "Greece", "Portugal", "Sweden"))
...
q_top_EU <- glue::glue_collapse(M_single_EU$UT,  sep = ' OR ') # querry for LA single-author paper 
```
We used the previous querry to find the single article authors in WoS

```{r}
files <- here::here("data", "processed", "EU_cited_articles", c("01_citation_EU.txt", "02_citation_EU.txt", "03_citation_EU.txt", "04_citation_EU.txt", "05_citation_EU.txt"))
...
tab_citation_EU <- data.frame(countries = names(countries_cite_EU), 
                              LA.citation = as.vector(countries_cite_EU), 
                              LA.citation.prop = as.vector(prop_countries_cite_EU))
```
...
tab_all_citation <- 
  ...
  rename(LA.citation.total = LA.citation.y) %>% 
  rename(LA.citation.proportion = LA.citation.prop.y) %>% 
  rename(eu.citation = LA.citation) %>% 
  rename(eu.citation.prop = LA.citation.prop)
```
...
ggplot(data = tab_all_citation, aes(x = continent, y = usa.citation.prop)) +
  ...
  ylab("Proportion of citations") +
  xlab("Region") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 75, vjust = 1, hjust = 1))

la_citation_plot <- 
ggplot(data = tab_all_citation, aes(x = continent, y = LA.citation.proportion)) +
  geom_violin() +
  geom_dotplot(binaxis = 'y', stackdir = 'center', dotsize = 0.5) +
  stat_summary(fun = "mean",
               geom = "crossbar", 
...
plot_grid(usa_citation_plot, la_citation_plot, eu_citation_plot, ncol = 3)
