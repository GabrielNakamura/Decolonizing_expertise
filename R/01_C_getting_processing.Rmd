...
Most 10000 recent citations from top 235 single author papers from USA and Canada. These citation articles were extracted from WoS using the code above, we simply copied and paste the identifiers above in the browser using the web application of WoS to perform the search. This is the same procedure I performed above for LA region
...
#### Queryng papers for South Asia

```{r}

files <- here::here("data", "raw", "SouthAsia_top1000.txt")
M <- convert2df(file = files, dbsource = 'wos', format = "plaintext")
results_SouthAsia <- biblioAnalysis(M, sep = ";")
M_single_SouthAsia <- M[which(unlist(lapply(strsplit(x = M$AU, split = ";"), function(x) length(x))) == 1), ]
...
```{r}
southasia_names <- toupper(names_SouthAsia)
s_paper_southasia <- unique(unlist(lapply(southasia_names, function(x) grep(x, M_single_SouthAsia$C1)))) # filtering by single author paper from LA 
M_single_SouthAsia <- M_single_SouthAsia[s_paper_southasia, ] # unique single article papers from LA
q_top_SouthAsia <- glue::glue("UT = (", glue::glue_collapse(M_single_SouthAsia$UT,  sep = ' OR '), ")") # querry for LA single-author paper 
```

Finally, I downloaded the information for 7000 studies that cited the top single author papers in South Asia

```{r}
files <- here::here("data", "processed", "SouthAsia_citing_articles", c("01_citing_SouthAsia.txt", "02_citing_SouthAsia.txt", "03_citing_SouthAsia.txt", "04_citing_SouthAsia.txt", "05_citing_SouthAsia.txt", "06_citing_SouthAsia.txt", "07_citing_SouthAsia.txt"))
M_citations_SouthAsia <- convert2df(file = files, dbsource = 'wos', format = "plaintext") # this will take a few minutes
results_citation_SouthAsia <- biblioAnalysis(M_citations_SouthAsia, sep = ";")
countries_cite_SouthAsia <- results_citation_SouthAsia$Countries[order(results_citation_SouthAsia$Countries, decreasing = TRUE)]
prop_countries_cite_SouthAsia <- (countries_cite_SouthAsia)/sum(countries_cite_SouthAsia)
tab_citation_SouthAsia <- data.frame(countries = names(countries_cite_SouthAsia), 
                              SouthAsia.citation = as.vector(countries_cite_SouthAsia), 
                              SouthAsia.citation.prop = as.vector(prop_countries_cite_SouthAsia))
```
...
```
#### Queryng papers for Sub-sahara region

```{r}

files <- here::here("data", "raw", "SubSahara_top1000.txt")
M <- convert2df(file = files, dbsource = 'wos', format = "plaintext")
results_SubSahara <- biblioAnalysis(M, sep = ";")
M_single_SubSahara <- M[which(unlist(lapply(strsplit(x = M$AU, split = ";"), function(x) length(x))) == 1), ]
...
```{r}
subsahara_names <- toupper(names_SubSahara)
s_paper_subsahara <- unique(unlist(lapply(subsahara_names, function(x) grep(x, M_single_SubSahara$C1)))) # filtering by single author paper from Middle and east africa
M_single_SubSahara <- M_single_SubSahara[s_paper_subsahara, ] # unique single article papers from LA
q_top_subsahara <- glue::glue("UT = (", glue::glue_collapse(M_single_SubSahara$UT,  sep = ' OR '), ")") # query for LA single-author paper 
```

We used the previous query to find the single article authors in WoS and downloaded the information of the ~4000 papers citing East Middle Africa single author papers

```{r}
files <- here::here("data", "processed", "Sahara_citing_articles", c("01_citing_sahara.txt", "02_citing_sahara.txt", "03_citing_sahara.txt", "04_citing_sahara.txt", "05_citing_sahara.txt"))
M_citations_Sahara <- convert2df(file = files, dbsource = 'wos', format = "plaintext") # this will take a few minutes
results_citation_Sahara <- biblioAnalysis(M_citations_Sahara, sep = ";")
countries_cite_Sahara <- results_citation_Sahara$Countries[order(results_citation_Sahara$Countries, decreasing = TRUE)]
prop_countries_cite_Sahara <- (countries_cite_Sahara)/sum(countries_cite_Sahara)
tab_citation_Sahara <- data.frame(countries = names(countries_cite_Sahara), 
                              sahara.citation = as.vector(countries_cite_Sahara), 
                              sahara.citation.prop = as.vector(prop_countries_cite_Sahara))
```
...
tab_all_citation <- 
  ...
  right_join(tab_citation_Sahara, by = "countries")

```

Saving M type matrices to perform rarefaction and also citation matrices for further analysis

```{r}
saveRDS(M_citations_EastAsia, here::here("data", "processed", "M_EastAsia.rds"))
saveRDS(M_citations_EU, here::here("data", "processed", "M_EU.rds"))
saveRDS(M_citations_la, here::here("data", "processed", "M_LA.rds"))
saveRDS(M_citations_MiddleAfrica, here::here("data", "processed", "M_MiddleAfrica.rds"))
saveRDS(M_citations_Sahara, here::here("data", "processed", "M_Sahara.rds"))
saveRDS(M_citations_usa_can, here::here("data", "processed", "M_USA.rds"))
saveRDS(M_citations_usa_can, here::here("data", "processed", "M_USA.rds"))
...
First I manually corrected some continent names
...
tab_all_citation[tab_all_citation$countries %in% names_SubSahara, "continent"] <- "Sahara"

# manually fixing some classifications
tab_all_citation[which(is.na(tab_all_citation$continent) == TRUE), "continent"] <- 
  c("South Asia", "East Asia", "Latin America", "South Asia", "Europe", "Middle East & Africa", "Sahara", "East Asia", "South Asia", "South Asia")
...
### citation bias with Pielou index
...
H_sahara <- diversity(tab_all_citation$sahara.citation[!is.na(tab_all_citation$sahara.citation)], index = "shannon")

H_mafrica <- diversity(tab_all_citation$middle.citation[!is.na(tab_all_citation$middle.citation)], index = "shannon")
...
J_sahara <- H_sahara/log(length(tab_all_citation$sahara.citation[!is.na(tab_all_citation$sahara.citation)]))

J_mafrica <- H_mafrica/log(length(tab_all_citation$middle.citation[!is.na(tab_all_citation$middle.citation)]))
