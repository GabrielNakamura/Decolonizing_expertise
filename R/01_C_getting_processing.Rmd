...
M <- convert2df(file = files, dbsource = 'wos', format = "plaintext")
results <- biblioAnalysis(M, sep = ";")
M_single_USA_CAN <- M[which(unlist(lapply(strsplit(x = M$AU, split = ";"), function(x) length(x))) == 1), ]
head(M_single_USA_CAN)
```

Since the query returns some single author papers from authors in which the affiliation does not correspond to a US or Can , I applied a second filter to detect only author's affiliations that contains at least one of Canada or US
...
```
Most 5000 recent citations from top 235 single author papers
...
M_citations_usa_can <- convert2df(file = files, dbsource = 'wos', format = "plaintext")
results_citation_usa_can <- biblioAnalysis(M_citations_usa_can, sep = ";")
(results_citation_usa_can$Countries[order(results_citation_usa_can$Countries, decreasing = TRUE)])/sum(results_citation_usa_can$Countries[order(results_citation_usa_can$Countries, decreasing = TRUE)])
```
