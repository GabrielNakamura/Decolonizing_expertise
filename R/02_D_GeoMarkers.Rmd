...
zoo_sa <- lapply(toupper(zoogeo), function(x) grep(x, M_SA$TI, fixed = T))
count_geo_sa <- lapply(geo_sa, function(x) ifelse(length(x) == 0, 0, length(x)))
count_zoo_sa <- lapply(zoo_sa, function(x) ifelse(length(x) == 0, 0, length(x)))
names(count_geo_sa) <- names_SouthAsia
names(count_zoo_sa) <- zoogeo
count_geo_sa <- unlist(count_geo_sa)
count_zoo_sa <- unlist(count_zoo_sa)
...
df_count_southasia <- data.frame(admin = names(count_geo_sa), count = as.numeric(count_geo_sa))
sf_count_sa <- 
  sf_southasia %>% 
  right_join(df_count_southasia, by = "admin") %>% 
  st_transform(crs = "+proj=robin")
...
sf_all <- rbind(sf_count_africa, sf_count_ea, sf_count_sa, sf_count_eu, sf_count_la, sf_count_NAfrica, sf_count_us)
...
Making maps showing the number of geographical markers for each WB region
