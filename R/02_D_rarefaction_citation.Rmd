...
viol_usacan <- 
  ...
  labs(subtitle = "USA/Canada", title = "") +
...
viol_sahara <- 
  ...
  xlab("") +
  ylab("Citations") +
  scale_y_continuous(breaks = c(seq(from = 50, to = 250, by = 75))) +
  ...
  theme(panel.background = element_rect(fill = "transparent"),
        plot.margin = unit(c(0.1, 0.1, 0.1, 0.1), "mm"),
        axis.text.x = element_text(family = "Times", color = "black", size = 7.5),
        legend.position = "none",
        legend.text = element_text(family = "Times", color = "black", size = 8),
        panel.border = element_blank())
...
```{r}
legend <- cowplot::get_legend(viol_la + theme(legend.position = "bottom")) 
all_violin <- 
  cowplot::plot_grid(viol_la, viol_usacan, viol_eastasia, viol_europe, viol_sahara, viol_middleafrica, ncol = 3)
plot_violin <- cowplot::plot_grid(all_violin, legend, ncol = 1, rel_heights = c(2, 0.5))
